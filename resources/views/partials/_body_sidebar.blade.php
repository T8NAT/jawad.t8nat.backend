@php
    ...
    $MyNavBar = \Menu::make('MenuList', function ($menu) use($url){
        ...
        $menu->add('<span>'.__('message.riderequest').'</span>'. ($new_ride_request > 0 ? '<span class="badge bg-primary p-1 mr-3">'.$new_ride_request.'</span>' : '') , [ 'class' => '' ])
            ...
            $menu->riderequest->add('<span>'.__('message.list_form_title',['form' => __('message.new_ride')]).'</span>', ['class' => 'sidebar-layout' ,'route' => ['riderequest.index', 'riderequest_type' => 'new_ride_requested']])
                ->data('permission', 'riderequest list')
                ->prepend('<i class="fas fa-list mr-1"></i>'.($new_ride_request > 0 ? '<span class="badge bg-primary p-1">'.$new_ride_request.'</span>' : ''))
                ->link->attr(['class' => '']);
        ...
        $menu->add('<span>'.__('message.complaint').'</span>'. ($pending_complaint > 0 ? '<span class="badge bg-primary p-1 mr-3">'.$pending_complaint.'</span>' : '') ,[ 'class' => ''])
            ...
            $menu->complaint->add('<span>'.__('message.list_form_title',['form' => __('message.pending')]).'</span>', ['class' => 'sidebar-layout' ,'route' => [ 'complaint.index', 'complaint_type' => 'pending' ]])
                ->data('permission', 'complaint list')
                ->prepend('<i class="fas fa-list"></i>'. ($pending_complaint > 0 ? '<span class="badge bg-primary p-1">'.$pending_complaint.'</span>' : ''))
                ->link->attr(['class' => '']);
        ...
        $menu->add('<span>'.__('message.withdrawrequest').'</span>'.($pending_withdraw_request > 0 ? '<span class="badge bg-primary p-1 mr-3">'.$pending_withdraw_request.'</span>' : ''), ['class' => ''])
            ...
            $menu->withdrawrequest->add('<span>'.__('message.list_form_title',['form' => __('message.pending')]).'</span>', ['class' => 'sidebar-layout' ,'route' => ['withdrawrequest.index','withdraw_type'=>'pending']])
                ->data('permission', 'withdrawrequest list')
                ->prepend('<i class="fas fa-list"></i>'.($pending_withdraw_request > 0 ? '<span class="badge bg-primary p-1 mr-3">'.$pending_withdraw_request.'</span>' : ''))
                ->link->attr(['class' => '']);
        ...
        $menu->add('<span>'.__('message.report',['name' => '']).'</span>', ['class' => ''])
            ...
            $menu->report->add('<span>'.__('message.driver_earning').'</span>', ['class' => ( request()->is('driver-earning') || request()->is('driver-earning/*') ) ? 'sidebar-layout active' : 'sidebar-layout', 'route' => 'driver.earning.list'])
